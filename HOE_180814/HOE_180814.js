...
var button_skip_x, button_skip_y;
var button_skip_width = 100; 
var button_skip_height = 50; 
...
function setup() {
  ...
  button3_rect_x = 25;
  button3_rect_y = windowHeight - 117;
  
  button_skip_x = windowWidth/2 - button_skip_width/2;
  button_skip_y = windowHeight/2 - button_skip_height/2 + 200;
...
function show_screen_8() {
  ...
  textStyle(BOLD);
  text(line_4, windowWidth/2 - windowWidth/4, windowHeight/2 - windowHeight/4 + vert_step*5, windowWidth/2, windowHeight/2); 
...
function show_screen_9() {
  ...
  textSize(13);
  textStyle(NORMAL);
  text(manifesto_de, 50, 50, windowWidth/3 - 50, windowHeight - 50); 
  textSize(14);
  text(manifesto_en, windowWidth - windowWidth/3, 50, windowWidth/3 - 50, windowHeight - 50); 
}
...
function show_screen_final() {
  ...
  image(drawing, windowWidth/2 + windowWidth/4 + 50 + 2*drag_x, windowHeight/2 + windowHeight/8 - 50 + 2*drag_y, drawing.width*ratio*drawing_scale, windowHeight*drawing_scale);
  ...
  textStyle(BOLD);
  text("Haus ohne Eigenschaften", 50, 50, windowWidth/3, windowHeight/4);
  textStyle(NORMAL);
  text("House Without Properties", 50, 70, windowWidth/3, windowHeight/4);
  textStyle(ITALIC);
  text(text_code, 50, 110, windowWidth/3, windowHeight/4);
  textStyle(BOLD);
  text(text_description_de, 50, windowHeight/2 - windowHeight/4, windowWidth/3 - 50, windowHeight/4 + 25);
  textStyle(NORMAL);
  text(text_description_en, 50, windowHeight/2 + 25, windowWidth/3 - 50, windowHeight/4 + 25); 

  //textSize(10);
  //text(manifesto_de, 50, windowHeight/2 - windowHeight/4, windowWidth/3 - 50, windowHeight/2 + 50); 
  
  textSize(14);
  textStyle(BOLD);
  text("RESET", 50, windowHeight - 100, windowWidth/3 - 50, 50); 
  
  textSize(10);
  textStyle(ITALIC);
  var credits = "Created by Céline Bessire, Guillermo Dürig, Luka Piškorec, Mathias Winter and Nemanja Zimonjić, 2018";
  text(credits, 50, windowHeight - 50, windowWidth/2, 50); 
  
  //text("CLICK ON A DRAWING TO ZOOM", 2*windowWidth/3,  windowHeight - 50, windowWidth/2, 50); 
  
  textFont('Webdings');
  textSize(30);
  textStyle(NORMAL);
  //text("L", 2*windowWidth/3,  windowHeight - 50, windowWidth/2, 50);
}
...
function mousePressed() {
  if ((button1_over == true)||(button2_over == true)||(button3_over == true)||(button4_over == true)||(button1_rect_over == true)||(button2_rect_over == true)||(button3_rect_over == true)||(button_skip_over == true)) {
    screen_nr += 1;
...
function update_text_input() {
  ...
  if (overButtonRect(button_skip_x, button_skip_y, button_skip_width, button_skip_height) ) {button_skip_over = true;} else {button_skip_over = false;}
  
  strokeWeight(10); noFill();
  c = 0;
  if (button_skip_over) {rect(button_skip_x, button_skip_y, button_skip_width, button_skip_height); lastInput = "GENERIC";}
  
  //test the button
  //rect(button_skip_x, button_skip_y, button_skip_width, button_skip_height);
