...
THREE.DitherShader = {
    ...
    "float Scale = 1.0;", //original was 1.0
      ...
      // custom dither shader with larger dither pixel size and no fragmentation of pattern
      "vec2 xy = gl_FragCoord.xy * Scale;",
			"int x = int(mod(xy.x, 16.0));",
			"int y = int(mod(xy.y, 16.0));",
      "int a = int(floor(float(x)/2.0));",
      "int b = int(floor(float(y)/2.0));",
