 ...
 * original code by @oosmoxiecode
 */

THREE.MonocellShader = {

	uniforms: {

		"tDiffuse": { type: "t", value: null },
		"grayScale": { type: "i", value: 0 },
    "pixelScale": { type: "f", value: 1.0 }, //scale of the dither effect, loaded from outside
    ...
    "uniform float pixelScale;",
		...
		"varying vec2 vUv;",
    "float Scale = 1.0;", //original was 1.0
      ...
      // original dither shader works in 1 pix resolution
			//"vec2 xy = gl_FragCoord.xy * Scale;",
			//"int x = int(mod(xy.x, 8.0));",
			//"int y = int(mod(xy.y, 8.0));",
      //

      // custom dither shader with larger dither pixel size
      "vec2 xy = gl_FragCoord.xy * Scale;",
			"int x = int(mod(xy.x, 8.0 * pixelScale));",
			"int y = int(mod(xy.y, 8.0 * pixelScale));",
      "int a = int(floor(float(x)/pixelScale));", //default was int(floor(float(x)/2.0));
      "int b = int(floor(float(y)/pixelScale));", //default was int(floor(float(x)/2.0));
			...
			"if (grayScale == 0) {",
				...
				"vec3 finalRGB;",
				"finalRGB.r = find_closest(a, b, rgb.r);",
				"finalRGB.g = find_closest(a, b, rgb.g);",
				"finalRGB.b = find_closest(a, b, rgb.b);",
        ...
        "float final = find_closest(a, b, grayscale);",
